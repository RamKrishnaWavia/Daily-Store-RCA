...
if uploaded_file:
    ...
    # --- STATUS CLASSIFIER (Based on Latest NOI Rules) ---
    ...
    if not impact_data.empty:
        ...
        st.success("✅ No operational impacts recorded for late orders.")
    ...
    with tab1: # Tab 1: City Summary
        st.subheader("City Productivity & Status Stats")
        ...
        st.dataframe(city_final[['city_name', 'Orders', 'Open', 'Bin', 'OFD', 'Delivered', 'Cancelled', 'On_Time', 'Late', 'Late %', 'Avg Bin Wait']].sort_values('Late %', ascending=False), width='stretch')

    with tab2: # Tab 2: Delivery Slabs
        st.subheader("Delivery Timing Summary")
        def get_slab(t):
        ...
        ss1.write(f"🚀 **Peak Slab:** {slab_view.loc[slab_view['Orders'].idxmax(), 'Slab']}")
        ss2.write(f"✅ **Early Delivery Success:** {((df_act['Slab'] == '1. Before 7:00 AM').mean()*100):.1f}%")
        
        st.dataframe(slab_view, width='stretch')
        st.bar_chart(slab_view.set_index('Slab')['Orders'])

    with tab3: # Tab 3: Store RCA
        ...
        sts2.write(f"⏱️ **Store Wait Time:** Avg: {int(store_base['Avg_Wait'].mean())}m")
    ...
    with tab4: # Tab 4: Route Analysis
        ...
        # Requirements: Average Societies per Route
    ...
    with tab5: # Tab 5: CEE Performance
        ...
        slab_mode = st.radio("Slab View By:", ["City", "Store"], horizontal=True)
    ...
    with tab6: # Tab 6: Society Analysis
        ...
        sos1.write(f"📦 **Avg Orders/Society:** {int(soc_view['Orders'].mean())}")
        sos2.write(f"🛵 **Avg CEEs/Society:** {soc_view['CEEs'].mean():.1f}")
        st.dataframe(soc_view.sort_values('Orders', ascending=False), width='stretch')

    with tab7: # Tab 7: Order Detail
        od = df_act.copy()
