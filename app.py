...
if uploaded_file:
    ...
    with tab1:
        ...
        city_act = df_act_filtered.groupby('city_name').agg(Actionable_Orders=('order_id', 'count'), On_Time_Orders=('On_Time', 'sum'), Late_Orders=('Late', 'sum'), Avg_Bin_Wait=('Wait_Time_Mins', 'mean'), DC_Delay=('DC Delay', 'sum'), Pick_Delay=('Pick Delay', 'sum'), CEE_Late=('CEE Late', 'sum')).reset_index()
        ...
        st.dataframe(city_data[['City', 'Total_Orders', 'Cancelled', 'Actionable_Orders', 'On_Time_Orders', 'Late %', 'Late_Orders', 'Avg Bin Wait', 'DC_Delay', 'Pick_Delay', 'CEE_Late']], width='stretch')
    ...
    with tab2:
        ...
        slab_data = df_act_filtered[df_act_filtered['Slab'] != "Undelivered"].groupby('Slab').agg(Orders=('order_id', 'count'), Stores=('sa_name', 'nunique'), Societies=('society_id', 'nunique')).reset_index()
        st.dataframe(slab_data, width='stretch')
    ...
    with tab3:
        ...
        st_act = df_act_filtered.groupby('sa_name').agg(Actionable_Orders=('order_id', 'count'), On_Time_Orders=('On_Time', 'sum'), Late_Orders=('Late', 'sum'), DC_Delay=('DC Delay', 'sum'), Pick_Delay=('Pick Delay', 'sum'), CEE_Late=('CEE Late', 'sum'), Avg_Bin_Wait=('Wait_Time_Mins', 'mean')).reset_index()
        ...
        st.dataframe(st_data[['sa_name', 'Total_Orders', 'Cancelled', 'Actionable_Orders', 'On_Time_Orders', 'Total_Societies', 'Late %', 'Primary RCA', 'Avg Bin Wait', 'Late_Orders', 'DC_Delay', 'Pick_Delay', 'CEE_Late']].sort_values('Late %', ascending=False), width='stretch')

    with tab4:
        st.subheader(f"Route Analysis: {selected_city}")
        rt_data = df_act_filtered.groupby(['route_id', 'sa_name']).agg(CEEs=('cee_id', 'nunique'), Orders=('order_id', 'count'), Societies=('society_id', 'nunique'), Late_Orders=('Late', 'sum'), First_Bin=('order_binned_time', 'min'), Last_Del=('order_delivered_time', 'max'), Avg_Wait=('Wait_Time_Mins', 'mean')).reset_index()
        ...
        st.dataframe(rt_data[['Route ID', 'Store', 'CEEs', 'Orders', 'Societies', 'Late Rate', 'First binned', 'Avg Bin Wait', 'Last delivered', 'Primary RCA']].sort_values('Orders', ascending=False), width='stretch')

    with tab5:
        st.subheader(f"CEE Performance: {selected_city}")
        cee_data = df_act_filtered.groupby(['cee_name', 'cee_id', 'sa_name']).agg(Routes=('route_id', 'nunique'), Orders=('order_id', 'count'), Societies=('society_id', 'nunique'), Late_Orders=('Late', 'sum'), First_Asg=('assignment_to_Cee_time', 'min'), Last_Del=('order_delivered_time', 'max')).reset_index()
        ...
        # FIXED: Sort the full dataframe FIRST, then select columns for display
        cee_sorted = cee_data.sort_values('Late_Rate_Val', ascending=False)
        st.dataframe(cee_sorted[['CEE name', 'Store', 'Routes', 'Orders', 'Societies', 'On-time', 'Late rate', 'First assigned', 'Last delivered', 'Flags']], width='stretch')

    with tab6:
        st.subheader(f"Society Analysis: {selected_city}")
        soc_data = df_act_filtered.groupby(['society_id', 'sa_name']).agg(Orders=('order_id', 'count'), Late_Orders=('Late', 'sum'), CEEs=('cee_id', 'nunique'), Routes=('route_id', 'nunique'), First_Del=('order_delivered_time', 'min'), Last_Del=('order_delivered_time', 'max')).reset_index()
        ...
        st.dataframe(soc_data[['Society ID', 'Store', 'Orders', 'Late rate', 'CEEs', 'No of Routes', 'First delivered', 'Last delivered', 'Primary RCA']].sort_values('Orders', ascending=False), width='stretch')
    ...
    with tab7:
        ...
        st.dataframe(od[['Order ID', 'Store', 'CEE Name', 'Route', 'Status', 'Binned', 'Assigned', 'Delivered', 'Bin→Assign', 'Assign→Deliver', 'RCA']], width='stretch')
