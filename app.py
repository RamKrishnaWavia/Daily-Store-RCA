...
if uploaded_file:
    ...
    # Wait Start Logic: Clock starts at 4 AM or Bin Time (whichever is later)
    ...
    with tab3: # Store RCA
        ...
        st_act = df_act.groupby('sa_name').agg(Actionable_Orders=('order_id', 'count'), On_Time_Orders=('On_Time', 'sum'), Late_Orders=('Late', 'sum'), Avg_Wait=('Effective_Wait_Mins', 'mean')).reset_index()
    ...
    with tab5: # CEE Performance
        ...
        with col_dist2:
            st.bar_chart(trip_dist.set_index('Trips Completed'))
        st.divider()
        def get_cee_slab(t):
    ...
    with tab7: # Order Detail
        ...
        od_cols = ['order_id', 'sa_name', 'cee_name', 'Bin_Time', 'Assign_Time', 'Deliv_Time', 'Primary_RCA']
